...
if __name__ == "__main__":
    ...
    with tf.name_scope("FC2"):
        w4 = weight([128, 256])
        b4 = bias([256])
        hidden2 = tf.nn.relu(tf.matmul(hidden1, w4) + b4)
        dropout1 = tf.nn.dropout(hidden2, rate=0.2)
    with tf.name_scope("OutputLayer"):
        w4 = weight([256, 10])
        b4 = bias([10])
        logits = tf.matmul(dropout1, w4) + b4
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(EPOCHS):
            ...
            loss, acc, val_result = sess.run([cross_entropy_loss, accuracy, merged], feed_dict={features: x_val, y_label: y_val})
            train_result = sess.run(merged, feed_dict={features: x_train, y_label: y_train})
            val_writer.add_summary(val_result, epoch)
